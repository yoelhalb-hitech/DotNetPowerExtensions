...
public class MustIinitializeAccessibilityNotLessThanConstructor : MustInitializeAnalyzerBase
{
    ...
    private void AnalyzeSymbol(SymbolAnalysisContext context, INamedTypeSymbol[] mustInitializeSymbols)
    {
        try
        {
            ...
            var ctors = symbol.ContainingType.Constructors
                    // Filter out a spceial protected ctor generated by the compiler for records
                    .Where(c => c.DeclaredAccessibility != Accessibility.Protected
                            || !c.DeclaringSyntaxReferences.IsEmpty
                            || c.Parameters.Length != 1
                            || c.Parameters.FirstOrDefault()?.Type.IsEqualTo(symbol.ContainingType) == false);
            var hasAllCtorAccessibility = ctors.All(c => predicate(accesibility, c.DeclaredAccessibility));
