...
internal class LocalInitializerQuickInfoProvider_Tests
{
    ...
    public async Task Test()
    {
        ...
        Assert.That(results.Sections.Count, Is.EqualTo(3));
        ...
        Assert.That(results.Sections[2].Text, Is.EqualTo("\r\nTTTT"));
    ...
    [Test]
    public async Task Test_MightRequire()
    {
        var code = @"
        using SequelPay.DotNetPowerExtensions;
        ...
        public class TestClass
        {
        	...
        	public static void Test(ILocalFactory<TestClass> factory)
        	{
        		var result = factory.Create(new { Testing = ""34"" });
        	}
        }
        ";
        ...
        var qi = QuickInfoService.GetService(document)!;
        var results = await qi.GetQuickInfoAsync(document, code.LastIndexOf("Testing", StringComparison.Ordinal)).ConfigureAwait(false);

        Assert.That(results, Is.Not.Null);
        //Assert.That(results.Sections.Count, Is.EqualTo(2));
        Assert.That(results.Sections[0].Text, Is.EqualTo("string MightRequire<TestClass>.Testing"));
        //Assert.That(results.Tags.Count, Is.EqualTo(2));
        Assert.That(results.Tags[0], Is.EqualTo("MightRequire"));
        //Assert.That(results.Tags[1], Is.EqualTo("Public"));
    }
}
