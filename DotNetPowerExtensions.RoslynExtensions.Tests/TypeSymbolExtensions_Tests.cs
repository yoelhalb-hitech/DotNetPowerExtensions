...
public class SymbolExtensions_Tests
{
    ...
    [Test]
    public void Test_GetConstructorChain_WithThisAndBase_WhenMultipleProjects()
    {
        var source1 = """
        public class DeclareTypeBaseBase
        {
        ...
        var compilation1 = CSharpCompilation.Create("Test",
                                syntaxTrees: new[] { SyntaxFactory.ParseSyntaxTree(source1) },
        ...
        try
        {
            ...
            var compilation2 = CSharpCompilation.Create("Test1",
                                    syntaxTrees: new[] { SyntaxFactory.ParseSyntaxTree(source2) },
                                    references: new[] { MetadataReference.CreateFromFile(outputFile1), MetadataReference.CreateFromFile(typeof(object).Assembly.Location) },
                                    options: new CSharpCompilationOptions(OutputKind.DynamicallyLinkedLibrary));
            ...
            var semanticModel = CSharpCompilation.Create("Test2",
                                    syntaxTrees: new[] { tree },
                                    references: new[] { MetadataReference.CreateFromFile(outputFile1), MetadataReference.CreateFromFile(outputFile2),
                                                                                        MetadataReference.CreateFromFile(typeof(object).Assembly.Location) },
                                    options: new CSharpCompilationOptions(OutputKind.DynamicallyLinkedLibrary))
                                .GetSemanticModel(tree);
            using var stream3 = new MemoryStream();
            semanticModel.Compilation.Emit(stream3).Success.Should().BeTrue();
